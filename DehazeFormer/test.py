...
def test(test_loader, network, result_dir):
	...
	for idx, batch in enumerate(test_loader):
		...
		# Example: Resize input image to the nearest dimension divisible by 16
		new_height = (input.shape[2] // 16) * 16
		new_width = (input.shape[3] // 16) * 16
		input = resize(input, size=(new_height, new_width))
		target = resize(target, size=(new_height, new_width))
		...
		with torch.no_grad():
			...
			output = F.interpolate(output, size=(new_height, new_width), mode='bilinear', align_corners=False)
